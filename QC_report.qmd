...
# Path to outputs (default to results directory)
reprep_repool_summary_path <- file.path(results_dir,"reprep_repool_summary.csv")
polyclonal_information_path <- file.path(results_dir,"positive_control_polyclonal_info.csv")
neg_control_information_path <- file.path(results_dir,"negative_control_amplified_loci.csv")
filtered_allele_data_path <- file.path(results_dir,"allele_data_filtered.txt")
filtered_resmarker_table_path <- file.path(results_dir,"resistance_marker_module/resmarker_table_filtered.txt")
filtered_resmarker_microhaplotype_table_path <- file.path(results_dir,"resistance_marker_module/resmarker_microhaplotype_table_filtered.txt")
```

```{r}
# If TRUE, trims extra sequencing-related information from SampleID to match the manifest format
# (if your manifest sample names match the FASTQ file names, including _SX_L001, for example, set to FALSE)
...
p <- ggplot(result, aes(x = SampleID, y = Count, fill = Category, text = paste("Count:", Count))) +
  ...
  labs(title = "Alleles per Locus for Positive Controls", x = "Sample ID", y = "Number of Loci") +
  theme_minimal() +
  scale_fill_manual(values = c("Monoallelic" = "Gray", "2 Alleles" = "orangered2", ">2 Alleles" = "turquoise3")) + 
  theme(
...
ggplot(amplicons_negative) +
  ...
  geom_vline(aes(xintercept = read_filter, linetype = "Reads per allele filter"), color = "black") +
  scale_linetype_manual(name = NULL, values = c("Reads per allele filter" = "dashed"))

print(paste("Max. reads in any locus in negative controls:",max(amplicons_negative$OutputPostprocessing)))
```
...
cat("The following file was created\n")
cat(reprep_repool_summary_path)
```
...
## Save filtered allele and resistance marker tables
...
# Read in resmarker raw data with no other filtering applied
resmarker_table <- read.csv(resmarker_table_path, sep='\t')

# Calculate allele frequency
resmarker_table <- resmarker_table %>%
  group_by(SampleID, GeneID, Gene, CodonID) %>%
  mutate(WSAF = Reads / sum(Reads)) %>%
  ungroup()

# Filter to minimum read and within sample allele frequency
resmarker_table_filtered <- resmarker_table %>%
  filter(Reads > read_filter & WSAF > af_filter)%>%
  group_by(SampleID, GeneID, Gene, CodonID) %>%
  mutate(WSAF = Reads / sum(Reads)) %>%
  ungroup()

# Write out table
write.csv(resmarker_table_filtered, filtered_resmarker_table_path, row.names = FALSE)

# Read in resmarker_microhaplotype raw data with no other filtering applied
resmarker_microhaplotype_table <- read.csv(resmarker_microhaplotype_table_path, sep='\t')


# Calculate allele frequency
group_cols <- c("SampleID", "GeneID", "Gene", "Locus", "MicrohaplotypeCodonIDs", "RefMicrohap")
...
# Write out table
write.csv(resmarker_microhaplotype_table_filtered, filtered_resmarker_microhaplotype_table_path, row.names = FALSE)

cat("The following files were created\n")
cat(filtered_allele_data_path)
cat(filtered_resmarker_table_path)
cat(filtered_resmarker_microhaplotype_table_path)
```
